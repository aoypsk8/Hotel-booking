...
class RoomsController {
    static async getAll(req, res) {
        ...
        try {
            const query = 'SELECT tr.* ,tt.Type_name,tt.PriceDay,tt.PriceMonth,tt.PriceYear FROM tb_rooms AS tr JOIN tb_type AS tt';
    ...
    static async update(req, res) {
        ...
        try {
            const query = `UPDATE tb_rooms SET Type_ID = ?, Room_Number = ? WHERE Room_ID = ${Room_ID}`;
            connection.query(query, [Type_ID, Room_Number], (error, results) => {
                connection.end();
                if (error) {
                    return res.json({ message: "An error occurred", error });
