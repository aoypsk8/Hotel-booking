...
import { createRoom, deleteRoom, getAllRoom, getAllRoomWhereType, updateRoom, updateRoomStatus } from "./roomApi";
...
export const GetAllRoomWhereType = (id) => async (dispatch) => {
  try {
    const room = await getAllRoomWhereType(id);
    console.log(room);
    if (room.status == "ok") {
      dispatch(addRoom(room.data));
      return true;
...
export const UpdateRoomStatus =
  (Room_ID, status) =>
    async (dispatch) => {
      ...
      console.log(status);
      try {
        const room = await updateRoomStatus(Room_ID, status);
